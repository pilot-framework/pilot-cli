...
const TF_CLOUD_INIT: string = path.join(appRoot.toString(), '/aws/tf-cloud-init.pub')
const TERRAFORM_EXEC: string = path.join(appRoot.toString(), '/bin/terraform_1.0.2_linux_amd64/terraform ')
const SSH_DOCKER_WAYPOINT_INIT: string = path.join(appRoot.toString(), 'src/templates/ssh-docker-waypoint-init.yaml')
const EC2_KEY_PAIR: string = path.join(appRoot.toString(), '/aws/PilotKeyPair.pem')
